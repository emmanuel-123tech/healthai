...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        console.log("[v0] CSV text lengths:", {
          state: stateText.length,
          lga: lgaText.length,
          facility: facilityText.length,
        })
        ...
        console.log("[v0] Parsed data counts:", {
          state: stateParsed.length,
          lga: lgaParsed.length,
          facility: facilityParsed.length,
        })
  ...
  const filteredData = useMemo<FilteredDataset>(() => {
    ...
    // For LGA admins, filter by their LGA
    if (selectedLGA === "all") {
      return {
        lgaData: lgaData,
    ...
    console.log("[v0] Filtered data counts:", {
      lga: filtered.lgaData.length,
      facility: filtered.facilityData.length,
    })
  ...
  const getDashboardTitle = () => {
    if (currentUser?.role === "health_worker") {
      return `${currentUser.facility || "Facility"} Dashboard`
    }
    if (currentUser?.role === "facility_manager") {
      return `${currentUser.facility || "Facility"} Management Dashboard`
    }
    if (currentUser?.role === "lga_admin") {
      return `${currentUser.lga || "LGA"} Dashboard`
    }
    return "PHC Intelligence Hub"
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-background via-muted/20 to-background">
        <div className="text-center">
          ...
          <p className="text-lg font-medium text-foreground">Loading Health Intelligence</p>
      ...
      <div className="container mx-auto px-4 py-8 space-y-8">
        <div className="animate-fade-in space-y-4">
          ...
          <p className="mt-3 text-xl text-muted-foreground max-w-3xl leading-relaxed">
            {currentUser?.role === "health_worker" || currentUser?.role === "facility_manager"
              ? `Real-time health analytics and insights for ${currentUser.facility || "your facility"}`
              : `Real-time health analytics and predictive insights from ${uniqueFacilities} Primary Health Care facilities across ${uniqueLGAs} LGAs in Ondo State`}
          </p>
        </div>

        {!currentUser?.facility && (
          <Card className="p-6 border-0 shadow-lg bg-gradient-to-r from-primary/10 via-accent/10 to-primary/10">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div>
                <h3 className="font-semibold text-lg flex items-center gap-2 mb-2">
                  <Building2 className="h-5 w-5 text-primary" />
                  Ondo State Health Analytics
                </h3>
                <p className="text-sm text-muted-foreground">
                  Real-time PHC performance metrics and disease surveillance
                </p>
              </div>
            </div>
          </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card className="p-6 animate-slide-up card-hover border-0 shadow-lg bg-gradient-to-br from-card to-primary/5 relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <p className="text-xs text-muted-foreground">
                {currentUser?.role === "health_worker" || currentUser?.role === "facility_manager"
                  ? "At your facility"
                  : `Avg ${avgOPDPerFacility.toLocaleString()} per facility`}
              </p>
            </div>
          </Card>
          ...
          >
            ...
            <div className="relative">
              ...
              <p className="text-xs text-muted-foreground">
                {currentUser?.role === "health_worker" || currentUser?.role === "facility_manager"
                  ? `In ${currentUser?.lga || "your LGA"}`
                  : `Across ${uniqueLGAs} Local Government Areas`}
              </p>
            </div>
          </Card>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div>
              <h3 className="font-semibold text-lg flex items-center gap-2">
                ...
                {currentUser?.role === "health_worker" || currentUser?.role === "facility_manager"
                  ? "Your Facility Analytics"
                  : "Filter Analytics"}
              </h3>
              <p className="text-sm text-muted-foreground mt-1">
                {currentUser?.facility
                  ? `Showing data for ${currentUser.facility}`
                  : currentUser?.lga && currentUser.role !== "state_official"
                    ? `Showing data for ${currentUser.lga}`
                    : "Drill down by Local Government Area"}
              </p>
            </div>
            {canChangeLGA && (
